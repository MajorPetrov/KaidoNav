...
Java_app_kaidonav_editor_OsmOAuth_nativeGetOAuth2Url(JNIEnv * env, jclass)
{
...
JNIEXPORT jstring JNICALL
Java_app_kaidonav_editor_OsmOAuth_nativeAuthWithPassword(JNIEnv * env, jclass clazz,
...
JNIEXPORT jstring JNICALL
Java_app_kaidonav_editor_OsmOAuth_nativeAuthWithOAuth2Code(JNIEnv * env, jclass, jstring oauth2code)
{
  OsmOAuth auth = OsmOAuth::ServerAuth();
  try
  {
...
JNIEXPORT jstring JNICALL
Java_app_kaidonav_editor_OsmOAuth_nativeGetOsmUsername(JNIEnv * env, jclass, jstring oauthToken)
{
...
Java_app_kaidonav_editor_OsmOAuth_nativeGetOsmChangesetsCount(JNIEnv * env, jclass, jstring oauthToken)
{
  UserPreferences prefs;
  if (LoadOsmUserPreferences(jni::ToNativeString(env, oauthToken), prefs))
...
JNIEXPORT jstring JNICALL
Java_app_kaidonav_editor_OsmOAuth_nativeGetOsmProfilePictureUrl(JNIEnv * env, jclass, jstring oauthToken)
{
  UserPreferences prefs;
  if (LoadOsmUserPreferences(jni::ToNativeString(env, oauthToken), prefs))
...
JNIEXPORT jstring JNICALL
Java_app_kaidonav_editor_OsmOAuth_nativeGetHistoryUrl(JNIEnv * env, jclass, jstring user)
{
  return jni::ToJavaString(env, OsmOAuth::ServerAuth().GetHistoryURL(jni::ToNativeString(env, user)));
}

JNIEXPORT jstring JNICALL
Java_app_kaidonav_editor_OsmOAuth_nativeGetNotesUrl(JNIEnv * env, jclass, jstring user)
{
