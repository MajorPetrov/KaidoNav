...
import app.kaidonav.routing.SingleLaneInfo;
import app.kaidonav.util.Graphics;
import app.kaidonav.widget.LanesDrawable;
...
public final class RoutingUtils
{
  ...
  public static Trip createTrip(@NonNull final CarContext context, @Nullable final RoutingInfo info, @Nullable MapObject endPoint)
  {
    ...
    if (!TextUtils.isEmpty(info.nextStreet))
      builder.addStep(createNextStep(context, info), createTravelEstimate(app.kaidonav.util.Distance.EMPTY, 0));
    return builder.build();
  }
  ...
  @NonNull
  private static TravelEstimate createTravelEstimate(@NonNull app.kaidonav.util.Distance distance, int time)
  {
